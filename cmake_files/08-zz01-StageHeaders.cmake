...
wxbgi_stage_headers_from_root("${GLFW_INCLUDE_ROOT}" "${GLFW_STAGE_PREFIX}" "GLFW")
...
if(WXBGI_ENABLE_WX AND NOT WXWIDGETS_CONFIG_ROOT STREQUAL "")
    ...
    if(NOT EXISTS "${_wx_setup_header}")
        file(GLOB_RECURSE _wx_setup_headers LIST_DIRECTORIES false
            "${WXWIDGETS_CONFIG_ROOT}/*setup.h")
        foreach(_wx_candidate IN LISTS _wx_setup_headers)
            get_filename_component(_wx_candidate_dir "${_wx_candidate}" DIRECTORY)
            get_filename_component(_wx_candidate_name "${_wx_candidate_dir}" NAME)
            if(_wx_candidate_name STREQUAL "wx")
                set(_wx_setup_header "${_wx_candidate}")
                break()
            endif()
        endforeach()
    endif()
